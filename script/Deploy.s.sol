...
///   2. deploy StillVault(token).
///   3. deploy StillHarvester(token, vault, factory, escrow, hook, poolManager, cap, cooldown).
///   4. transferCreatorFeeRecipient(token, harvester): from here on, fees can only go to the harvester.
///   5. buy SEED_ETH of STILL on the curve, deposit SEED_STILL into the vault, park the vSTILL at 0x...dEaD.
///
/// Env (see .env.example): PRIVATE_KEY, PONS_FACTORY, PONS_ESCROW, PONS_HOOK, POOL_MANAGER (optional),
/// LAUNCH_CONFIG_ID, CREATOR_TAX_BPS, MAX_BUY_WEI, COOLDOWN_SEC, SEED_ETH_WEI, SEED_STILL_WEI,
...
contract Deploy is Script {
    ...
    function run() external {
        ...
        o.vault = address(new StillVault(IERC20(o.token)));
        o.harvester = address(
            new StillHarvester(o.token, o.vault, address(c.factory), c.escrow, c.hook, c.poolManager, c.maxBuy, c.cooldown)
        );
    ...
    function _config() internal view returns (Cfg memory c) {
        ...
        c.seedStill = vm.envOr("SEED_STILL_WEI", uint256(1_000e18));
        ...
        c.salt = vm.envOr("SALT", keccak256(abi.encodePacked("STILL", c.deployer, block.timestamp)));
    }
    ...
    function _launch(Cfg memory c) internal returns (address token, address curve) {
        IPonsFactory.TokenParams memory params = IPonsFactory.TokenParams({
            ...
            description: vm.envOr(
                "TOKEN_DESCRIPTION",
                string(
                    "A vault on Robinhood Chain. Trading fees buy STILL into the vault. vSTILL just gets worth more. No claim step, no owner."
                )
            ),
    ...
    function _seed(Cfg memory c, Out memory o) internal {
        ...
        StillVault(o.vault).deposit(toDeposit, DEAD);
        console.log("seed buy STILL    ", got);
        console.log("seeded STILL      ", toDeposit);
    }
    ...
    function _log(Cfg memory c, Out memory o) internal pure {
        ...
        console.log("STILL token       ", o.token);
        console.log("curve             ", o.curve);
        console.log("StillVault        ", o.vault);
        console.log("StillHarvester    ", o.harvester);
    }
